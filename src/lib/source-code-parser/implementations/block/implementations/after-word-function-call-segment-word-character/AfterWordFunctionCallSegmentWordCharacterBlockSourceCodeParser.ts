...
export class AfterWordFunctionCallSegmentWordCharacterBlockSourceCodeParser extends BlockSourceCodeParser {
	public constructor(
	...
	public override feedWithClosingCurlyBracketCharacter(
		...
		const result = new ErrorFeedingResult(
			character,
			index,
			`Operator expected closing the block starting at index ${this.blockBuilder.computeSpanIndexesStartingIndex().toString()}.`,
		);
		return result;
	}
