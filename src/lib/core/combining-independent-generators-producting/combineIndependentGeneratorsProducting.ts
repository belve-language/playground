...
export function* combineIndependentGeneratorsProducting<Result1, Result2>(
	...
	for (const index of generateIndex2Diagonally()) {
		cache1 = ensureCacheUpTo<Result1>(cache1, generator1, index.in1);
		cache2 = ensureCacheUpTo<Result2>(cache2, generator2, index.in2);
		if (
			index.in1 < cache1.results.length
			&& index.in2 < cache2.results.length
		) {
			yield [
				cache1.results[index.in1] as (typeof cache1.results)[typeof index.in1],
				cache2.results[index.in2] as (typeof cache2.results)[typeof index.in2],
			];
		}
