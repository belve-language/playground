...
function* generateNewUnknownFunctionHeaderSegment(
	...
	for (const generatedNewUnknownName of generateNewUnknownName(depth)) {
		const generatedNewUnknownFunctionHeaderSegment =
			UnknownFunctionHeaderSegmentAbstractSyntaxTreeNode.create(
...
function* generateNewFunctionCallStatementFromFunctionHeader(
	...
	// );
	for (const generatedNewFunctionCallSegments of generateNewFunctionCallSegments(
		`${code}h`,
		depth + 1,
		functionHeader.children.segments,
		unknownsNames,
		...
		const generatedNewFunctionCallStatement =
			FunctionCallStatementAbstractSyntaxTreeNode.create(
				{segments: generatedNewFunctionCallSegments},
				new SpanIndexes(0, 0),
			);
		// if (
		// 	functionHeader.id === thisFunctionHeader.id
		// 	&& generatedNewFunctionCallSegments.every(
		// 		(generatedFunctionCallSegment, index) => {
		// 			const functionHeaderSegment = functionHeader.children.segments[index];
		// 			if (
		// 				generatedFunctionCallSegment
		// 					instanceof KnownFunctionCallSegmentAbstractSyntaxTreeNode
		// 				&& functionHeaderSegment
		// 					instanceof KnownFunctionHeaderSegmentAbstractSyntaxTreeNode
		// 				&& generatedFunctionCallSegment.name === functionHeaderSegment.name
		// 			) {
		// 				return true;
		// 			} else if (
		// 				generatedFunctionCallSegment
		// 					instanceof UnknownFunctionCallSegmentAbstractSyntaxTreeNode
		// 				&& functionHeaderSegment
		...
		// 			} else if (
		// 				generatedFunctionCallSegment
		// 					instanceof WordFunctionCallSegmentAbstractSyntaxTreeNode
		// 				&& functionHeaderSegment
		// 					instanceof WordFunctionHeaderSegmentAbstractSyntaxTreeNode
		// 				&& generatedFunctionCallSegment.children.word
...
export function* generateNewFunctionCallSegments(
	code: string,
	depth: number,
	...
	void
> {
	// console.log(`${code}|generateNewFunctionCallSegment`, depth);
	const [
	...
	if (secondFunctionHeaderSegment === undefined) {
		for (const generatedNewFunctionCallSegment of generateNewFunctionCallSegment(
			`${code}i`,
			depth + 1,
			firstFunctionHeaderSegment,
			unknownsNames,
			...
			const generatedNewFunctionCallSegments: readonly [
				SupportedFunctionCallSegmentAbstractSyntaxTreeNode,
			] = [generatedNewFunctionCallSegment];
			yield generatedNewFunctionCallSegments;
		}
	} else {
		for (const generatedNewFunctionCallSegment of combineDependentGeneratorsProducting(
			generateNewFunctionCallSegment(
				`${code}j`,
				depth + 1,
				firstFunctionHeaderSegment,
				unknownsNames,
			...
			function* (generatedNewFunctionCallSegment) {
				for (const deeperGeneratedNewFunctionCallSegmentsRest of generateNewFunctionCallSegments(
					`${code}k`,
					depth + 1,
					...
					const deeperGeneratedNewFunctionCallSegments: readonly [
						SupportedFunctionCallSegmentAbstractSyntaxTreeNode,
						SupportedFunctionCallSegmentAbstractSyntaxTreeNode,
						...(readonly SupportedFunctionCallSegmentAbstractSyntaxTreeNode[]),
					] = [
						generatedNewFunctionCallSegment,
						...deeperGeneratedNewFunctionCallSegmentsRest,
					];
					yield deeperGeneratedNewFunctionCallSegments;
				}
			},
		)) {
			yield generatedNewFunctionCallSegment;
		}
	}
}
function* generateNewFunctionCallSegment(
	code: string,
	depth: number,
	functionHeaderSegment: SupportedFunctionHeaderSegmentAbstractSyntaxTreeNode,
	unknownsNames: readonly string[],
	userVariablesNames: readonly string[],
): Generator<SupportedFunctionCallSegmentAbstractSyntaxTreeNode, void, void> {
	// console.log(`${code}|generateNewFunctionCallSegment`, depth);
	if (
		functionHeaderSegment
		instanceof KnownFunctionHeaderSegmentAbstractSyntaxTreeNode
	) {
		for (const generatedNewKnownCallSegment of generateNewKnownFunctionCallSegment(
			`${code}l`,
			depth + 1,
			userVariablesNames,
		)) {
		...
		for (const generatedNewUnknownCallSegment of generateNewUnknownFunctionCallSegment(
			`${code}m`,
			depth + 1,
			unknownsNames,
			userVariablesNames,
...
export function* generateNewKnownFunctionCallSegment(
	code: string,
	depth: number,
	userVariableNames: readonly string[],
): Generator<KnownFunctionCallSegmentAbstractSyntaxTreeNode, void, void> {
	// console.log(`${code}|generateNewKnownFunctionCallSegment`, depth);
	for (const generatedVariableName of combineIndependentGeneratorsInterleaving<string>(
		...
		const generatedNewKnownFunctionCallSegment =
			KnownFunctionCallSegmentAbstractSyntaxTreeNode.create(
				{name: generatedVariableName},
				new SpanIndexes(0, 0),
			);
		yield generatedNewKnownFunctionCallSegment;
	}
}
...
function* generateNewUnknownFunctionCallSegment(
	code: string,
	depth: number,
	unknownsNames: readonly string[],
	userVariablesNames: readonly string[],
): Generator<UnknownFunctionCallSegmentAbstractSyntaxTreeNode, void, void> {
	// console.log(`${code}|generateNewUnknownFunctionCallSegment`, depth);
