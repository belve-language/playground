<script lang="ts">
	...
	class Animator {
		...
		public constructor(characters: readonly string[]) {
			...
			this.animationData = {parser, reversedCharactersLeft: reversedCharacters};
		...
		public step(): void {
			const {parser, reversedCharactersLeft} = this.animationData;
			const [firstReversedCharactersLeft, ...restReversedCharactersLeft] =
				reversedCharactersLeft;
			if (typeof firstReversedCharactersLeft === "undefined") {
				return;
			}
			const index = reversedCharactersLeft.length - 1;
