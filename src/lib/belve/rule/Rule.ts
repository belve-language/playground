...
> {
	...
	public computePossibleFirstingTerminals(
		...
		if (alreadyCheckedRules.has(this)) {
			...
			>((accumulatedPossibleFirstingTerminals, expression) => {
				const ThisExpressionpossibleFirstingTerminals =
				...
				const newAccumulatedPossibleFirstingTerminals =
					accumulatedPossibleFirstingTerminals.union(
						ThisExpressionpossibleFirstingTerminals,
					);
	...
	public stringifyToBackusNaurForm(
		...
		// const stringifiedRule: string = `<${this.name}> ::= ${this.getRightExpressions(
		// 	ruleById,
		// )
		// 	.map((expression) => {
		// 		const stringifiedExpression: string =
		// 			expression.stringifyToBackusNaurForm();
